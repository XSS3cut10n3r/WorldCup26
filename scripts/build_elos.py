...
roughly its current market (Kalshi) rate. Writes elo.json.
    ...
    BUILD_WORKERS=1 python3 build_elos.py       # force single-process (original RNG path)
...
Pure stdlib. The Monte Carlo is split across CPU cores (multiprocessing): the
calibration loop stays sequential — each iteration nudges ratings off the last
iteration's result — but the thousands of independent sims *inside* each
iteration (and the big final validation pass) run in parallel and are summed
back together, which is statistically identical to running them one after
another. Runs faster still under PyPy with no code change. Set BUILD_WORKERS=1
to fall back to the original single-process behaviour (and exact RNG stream).
"""
...
EPS         = 1e-6
# 0 = use every CPU core (recommended); 1 = original single-process path.
...
def champion_freq(ctx, R, P, n, rng):
    """Single-process champion frequencies (used when BUILD_WORKERS=1, and as
    the building block each parallel worker runs on its own slice)."""
...
# --- parallel plumbing: each worker holds the static ctx + params, runs a
# --- slice of the sims for whatever ratings R the calibrator hands it, and
# --- returns raw champion counts to be summed in the parent.
_CTX = None
_P = None


def _init_worker(ctx, P):
    global _CTX, _P
    _CTX, _P = ctx, P
...
def _count_chunk(task):
    ...
    counts = {}
    for _ in range(n):
        c = simulate_once(_CTX, R, _P, rng)
        if c is not None:
            counts[c] = counts.get(c, 0) + 1
    return counts


def make_freq_fn(ctx, P, n, workers, pool, seeder):
    """Return freq_fn(R) -> {team: champ_probability}, summed across `workers`
    processes. The sims are split so the slices total exactly `n`; each call
    advances `seeder` so successive calibration iterations use fresh, indepen-
    dent draws (and the whole run is reproducible from the base seed)."""
    if workers == 1 or pool is None:
        rng = random.Random(seeder.randrange(2 ** 31 - 1))
        # one rng for the whole run, threaded across calls — matches the
        # original single-process stream when seeded identically.
        def freq_fn(R):
            return champion_freq(ctx, R, P, n, rng)
        return freq_fn

    base, rem = divmod(n, workers)
    chunk_sizes = [base + (1 if i < rem else 0) for i in range(workers)]
    all_teams = list(ctx["meta"].keys())

    def freq_fn(R):
        tasks = [(chunk_sizes[i], R, seeder.randrange(2 ** 31 - 1))
                 for i in range(workers)]
        total = {t: 0 for t in all_teams}
        for part in pool.map(_count_chunk, tasks):
            for t, c in part.items():
                total[t] += c
        return {t: total[t] / n for t in total}

    return freq_fn


def calibrate(ctx, target, P, freq_fn):
...
def main():
    ...
    # One generator seeded off the fixed base seed drives every per-worker
    # (and single-process) seed, so a given (seed, worker-count) is fully
    # reproducible. seeder is shared by calibration and the final pass.
    seeder = random.Random(SEED)

    pool = None
    try:
        if workers > 1:
            pool = Pool(workers, initializer=_init_worker, initargs=(ctx, PARAMS))
            print("Monte Carlo across %d worker(s)." % workers, file=sys.stderr)
        else:
            print("Monte Carlo single-process (BUILD_WORKERS=1).", file=sys.stderr)

        calib_freq = make_freq_fn(ctx, PARAMS, CALIB_SIMS, workers, pool, seeder)
        R, priced, pinned = calibrate(ctx, target, PARAMS, calib_freq)

        print("final %d-sim validation pass ..." % FINAL_SIMS)
        final_freq = make_freq_fn(ctx, PARAMS, FINAL_SIMS, workers, pool, seeder)
        freq = final_freq(R)
    finally:
        if pool is not None:
            pool.close()
